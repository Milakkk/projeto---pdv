...
function OrderTimeStatus({ order }: { order: Order }) {
  ...
  const { 
    ...
    // 1. Tempo de Espera (NEW): Tempo entre criação e início do preparo
    // Se o pedido está em NEW, o tempo de espera é calculado em tempo real (usando now).
    // Se o pedido já saiu de NEW, o tempo de espera é fixo (usando preparingStartTime).
    const currentPreparingStartTime = order.status === 'NEW' ? now : preparingStartTime;
    const tempoEsperaMs = currentPreparingStartTime - createdAt;
    const tempoEspera = Math.floor(tempoEsperaMs / 1000);
    ...
    const tempoEntrega = Math.floor(tempoEntregaMs / 1000);
                              ...
                              <div key={index} className="border border-gray-200 rounded-lg p-2 bg-white">
                                ...
                                <div className="space-y-1">
                                  {((item.productionUnits && item.productionUnits.length > 0) ? item.productionUnits : Array.from({ length: totalUnitsForItem }).map((_, idx) => ({ unitId: `direct-${idx}`, completedAt: undefined, operatorName: undefined, unitStatus: undefined as any }))).map((unit: any, unitIndex: number) => {
                                    ...
                                    let statusText = 'Em preparo';
                                    let statusColor = 'bg-yellow-500'; 
                                    
                                    if (item.skipKitchen) {
                                      statusText = isUnitDelivered ? 'Entregue' : 'Aguardando Entrega';
