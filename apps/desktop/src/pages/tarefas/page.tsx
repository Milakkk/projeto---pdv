...
import type { Task, Store, TaskStatus, TaskStatusKey } from '../../types';
...
export default function TarefasPage() {
  ...
  const handleUpdateStatus = (taskId: string, statusKey: TaskStatusKey) => {
    ...
    const isFinalStatus = Boolean(newStatus?.isFinal);
    svcUpdateTaskStatus(taskId, statusKey, isFinalStatus).then(() => {
      setTasks(prev => prev.map(t => t.id === taskId ? { ...t, status: statusKey, completedAt: isFinalStatus ? new Date() : undefined } : t));
    });
  };
  ...
  const handleSaveTask = (taskData: Omit<Task, 'id' | 'createdAt' | 'completedAt'>, isEditing: boolean) => {
    ...
    if (isEditing && editingTask) {
      ...
      upsertTask({ ...taskWithNames, id: editingTask.id, completedAt: isFinalStatus ? new Date() : undefined }).then(() => {
        setTasks(prev => prev.map(t => t.id === editingTask.id ? { ...t, ...taskWithNames, comments: taskData.comments || [], completedAt: isFinalStatus ? new Date() : undefined } : t));
      });
    } else {
      const payload = { ...taskWithNames, status: 'pending', comments: [] } as Omit<Task, 'id' | 'createdAt' | 'completedAt'>;
      upsertTask(payload).then((id) => {
        setTasks(prev => [{ ...taskWithNames, id, storeId: taskWithNames.storeId, status: 'pending', comments: [], createdAt: new Date() } as Task, ...prev]);
      });
    }
    setEditingTask(null);
  };
  ...
  const handleToggleComplete = (taskId: string) => {
    ...
    const isCompleted = current ? Boolean(taskStatuses.find(s => s.key === current.status)?.isFinal) : false;
    const newStatusKey = isCompleted ? 'pending' : 'completed';
    const newStatus = taskStatuses.find(s => s.key === newStatusKey);
    const isFinalStatus = Boolean(newStatus?.isFinal);
    svcUpdateTaskStatus(taskId, newStatusKey, isFinalStatus).then(() => {
      setTasks(prev => prev.map(t => t.id === taskId ? { ...t, status: newStatusKey, completedAt: isFinalStatus ? new Date() : undefined } : t));
    });
  };
  ...
  const confirmDelete = () => {
    if (taskToDelete) {
      svcDeleteTask(taskToDelete).then(() => {
