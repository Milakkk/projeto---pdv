...
function getDbPath() {
  // Se estiver no Electron, usar userData
  if (typeof app !== 'undefined' && app) {
    const userData = app.getPath('userData');
    const sharedDir = path.join(userData, 'shared');
    if (fs.existsSync(sharedDir)) {
      const dbPath = path.join(sharedDir, 'data.db');
  ...
  const possibleDbPaths = [
    ...
    path.join(process.env.APPDATA || '', 'PDV KDS Desktop', 'shared', 'data.db'), // Windows userData
...
if (!dbPath) {
  ...
  console.error('Procurando em:');
  console.error('  - Diretório atual:', path.join(process.cwd(), 'data.db'));
  console.error('  - Windows AppData:', path.join(process.env.APPDATA || '', 'PDV KDS Desktop', 'shared', 'data.db'));
  console.error('  - macOS:', path.join(process.env.HOME || '', 'Library', 'Application Support', 'PDV KDS Desktop', 'shared', 'data.db'));
  console.error('  - Linux:', path.join(process.env.HOME || '', '.config', 'PDV KDS Desktop', 'shared', 'data.db'));
  process.exit(1);
}
