...
import { OrderItem, Order, SavedCart, KitchenOperator, ProductionUnit, OperationalSession, Category, RequiredModifierGroup } from '../../../types';
...
import { CashMovement as CashMovementType } from './CashMovement'; // Importando o tipo correto
import { useAuth } from '../../../context/AuthContext'; // IMPORTAÇÃO CORRIGIDA
// Offline persistence services
import { createOrder, addItem as addOrderItem, addPayment as addOrderPayment, closeOrder } from '../../../offline/services/ordersService'
import { enqueueTicket } from '../../../offline/services/kdsService'
import { addMovement as addCashMovement } from '../../../offline/services/cashService'
...
export default function Cart({ items, onUpdateItem, onRemoveItem, onClearCart, onUpdateObservations, onLoadCart, onSaveOrders, operationalSession, onSetCashMovements, categories, onlineKitchenIds, isKitchenOnline }: CartProps) {
  ...
  }, [orderPassword, hasItemsWithMissingRequiredOptions, isMultiplePayment, calculateRemainingAmount, hasCashInBreakdown, paymentBreakdown, amountPaid, paidAmount, selectedPayment, isCashPayment, total]);
  ...
  const handleCheckout = useCallback(() => {
    ...
    if (cashSession && cashAmountForMovement > 0) {
      const cashMovement: CashMovementType = {
        id: Date.now().toString(),
        type: 'SALE',
        amount: cashAmountForMovement, // Valor líquido que entra no caixa
        description: `Venda - Pedido ${orderPin} (${newOrder.paymentMethod})`,
        timestamp: new Date(),
        orderId: newOrder.id,
        sessionId: cashSession.id // USANDO O ID DA SESSÃO ATIVA
      };
      
      // PERSISTÊNCIA CORRIGIDA: Usar a função de atualização passada via prop
    ...
    (async () => {
      try {
        const notes = `CREATED_BY=${user?.name || 'Caixa'}; PIN=${orderPin}; PASSWORD=${orderPassword.trim()}`
        const orderId = await createOrder({ notes, openedAt: now.toISOString() })

        // Persistir itens
        for (const item of itemsForOrder) {
          const productId = String(item.menuItem.id)
          const qty = item.quantity
          const unitPriceCents = Math.max(0, Math.round((item.unitPrice || 0) * 100))
          const obs = item.observations || null
          await addOrderItem({ orderId, productId, qty, unitPriceCents, notes: obs })
        }

        // Map de nomes de pagamento para enum do DB
        const toMethod = (name: string): 'cash' | 'pix' | 'debit' | 'credit' | 'voucher' => {
          const n = (name || '').toLowerCase()
          if (n.includes('dinheiro') || n === 'cash') return 'cash'
          if (n.includes('pix')) return 'pix'
          if (n.includes('débito') || n.includes('debito') || n === 'debit') return 'debit'
          if (n.includes('crédito') || n.includes('credito') || n === 'credit') return 'credit'
          if (n.includes('voucher') || n.includes('vale') || n.includes('vr')) return 'voucher'
          return 'cash'
        }
        ...
        if (isMultiplePayment) {
          ...
          for (const [methodName, value] of entries) {
            await addOrderPayment({
              orderId,
              method: toMethod(methodName),
              amountCents: Math.max(0, Math.round((value || 0) * 100)),
              changeCents: 0,
            })
          }
          // Troco em pagamentos múltiplos (apenas parte em dinheiro conta como troco)
          const cashAllocated = entries
            .filter(([m]) => m.toLowerCase().includes('dinheiro'))
            .reduce((s, [, a]) => s + (a || 0), 0)
          const changeCents = Math.max(0, Math.round((currentPaidAmount - cashAllocated) * 100))
          if (cashAllocated > 0 && changeCents > 0) {
            await addOrderPayment({ orderId, method: 'cash', amountCents: 0, changeCents })
          }
        } else {
          const method = toMethod(selectedPayment)
          const amountCents = Math.max(0, Math.round(currentTotal * 100))
          const changeCents = Math.max(0, Math.round((finalChangeAmount || 0) * 100))
          await addOrderPayment({ orderId, method, amountCents, changeCents })
        }

        // Enfileirar ticket de cozinha se necessário
        if (!onlyDirectDelivery) {
          await enqueueTicket({ orderId })
        }

        // Registrar movimento de caixa em DB (entrada por venda em dinheiro)
        if (cashSession && cashAmountForMovement > 0) {
          await addCashMovement({
            sessionId: cashSession.id,
            type: 'in',
            reason: `sale_${orderPin}`,
        ...
        console.error('Falha ao persistir pedido offline:', err)
      }
    })()
