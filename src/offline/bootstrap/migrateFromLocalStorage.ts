...
export async function runMigrationOnce() {
  ...
  const defaultUnitName = "Matriz";
  const defaultStationName = "Principal";

  // Escolhe/Cria uma unidade semente para associar dados migrados
  let seedUnitId: string | null = null;
  try {
    const existingUnits = await db.select().from(units);
    if (existingUnits && existingUnits.length) {
      seedUnitId = String(existingUnits[0].id);
    } else {
      seedUnitId = uuid();
    }
  } catch {
    // Se leitura falhar, gera um UUID para tentar inserção durante a transação
    seedUnitId = uuid();
  }
  ...
  const catRows = (lsCategories || []).map((c) => ({
    ...
    unitId: seedUnitId,
    defaultStation: defaultStationName,
    ...
    if (t) {
      ticketRows.push({
        ...
        status: ["queued", "prep", "ready", "done"].includes(t.status)
          ? t.status
          : "queued",
        station: t.station ? String(t.station) : defaultStationName,
  ...
  await db.transaction(async (tx) => {
    // Seed idempotente de unidade e estação padrão
    const existingUnits = await tx.select().from(units);
    if (!existingUnits.length) {
      await tx
        .insert(units)
        .values({ id: seedUnitId!, name: defaultUnitName, isActive: 1, createdAt: now, updatedAt: now })
        .onConflictDoNothing()
        .run();
    } else {
      // Garante seedUnitId alinhado com unidade já existente
      seedUnitId = String(existingUnits[0].id);
    }

    const existingStations = await tx.select().from(stations);
    const hasDefaultStation = existingStations.some(
      (s: any) => String(s.unitId) === String(seedUnitId) && String(s.name) === defaultStationName,
    );
    if (!hasDefaultStation) {
      await tx
        .insert(stations)
        .values({ id: uuid(), unitId: seedUnitId!, name: defaultStationName, isActive: 1, updatedAt: now })
        .onConflictDoNothing()
        .run();
    }
