...
import type { AuthContextType, User, Store, Role, Module } from '../types';
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const [preferredModule, setPreferredModule] = useLocalStorage<string | null>('preferred_module', null);
  ...
  const logout = useCallback(() => {
    ...
    setPreferredModule(null);
  }, [setStoredUser, setStoredStoreId, setPreferredModule]);
  ...
  const contextValue = useMemo(() => ({
    ...
    preferredModule,
    setPreferredModule,
  }), [storedUser, store, role, isAuthenticated, login, logout, hasPermission, preferredModule, setPreferredModule]);
