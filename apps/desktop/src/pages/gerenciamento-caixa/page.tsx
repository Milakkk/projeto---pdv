...
interface CashSession {
  ...
  difference?: number;
  status: 'OPEN' | 'CLOSED';
  // Separação de observações por fase da sessão
  openingNotes?: string; // Observações registradas na abertura
  closingNotes?: string; // Observações registradas no fechamento
  // Campo legado: algumas sessões antigas podem ter usado `notes` sem distinção
  notes?: string;
...
export default function GerenciamentoCaixaPage() {
  ...
  const totalSales = useMemo(() => {
    // Considera movimentos classificados como 'SALE' (mapeados quando motivo contém 'Venda')
      ...
      >
        {selectedCashSessionData && cashSessionDetails && (
          <div className="space-y-6">
            ...
            {(
              selectedCashSessionData.openingNotes ||
              selectedCashSessionData.closingNotes ||
              selectedCashSessionData.notes ||
              selectedCashSessionData.justification
            ) && (
              <div className="space-y-3">
                {(selectedCashSessionData.openingNotes || selectedCashSessionData.notes) && (
                  <div>
                    <h5 className="text-sm font-medium text-gray-700 mb-2">Observações da Abertura:</h5>
                ...
                {selectedCashSessionData.closingNotes && (
                  <div>
                    <h5 className="text-sm font-medium text-gray-700 mb-2">Observações do Fechamento:</h5>
                    <p className="text-sm text-gray-600 bg-gray-50 rounded-lg p-3">
                      {selectedCashSessionData.closingNotes}
                    </p>
                  </div>
